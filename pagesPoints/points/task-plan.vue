...
<script setup>
	...
	async function loadTaskPlanList(isLoadMore = false) {
		...
		try {
			const res = await http.post('/points-service/points/task/plan/page', params)
	...
	function handleUpdateStatus(item) {
		...
		http.put('/points-service/points/task/plan/updateStatus', {
			id: item.id,
			status: newStatus
		}).then(() => {
			uni.showToast({
	...
	function handleDelete(item) {
		uni.showModal({
			...
			success: function (res) {
				if (res.confirm) {
					http.delete(`/points-service/points/task/plan/delete?id=${item.id}`)
						.then(() => {
							uni.showToast({
								icon: 'success',
								title: '删除成功'
