...
<script setup>
	...
	function getTodoList() {
		http.post('/points-service/points/task/basics/list', {
			taskGroupId: 0
		}).then(res => {
	...
	function getDoneList() {
		...
		http.get('/points-service/points/task/basics/doneList?taskGroupId=0&currentPage=' + page.currentPage).then(res => {
			const data = res.data
	...
	function handleAddTask() {
		...
		http.post('/points-service/points/task/basics/add', {
			taskGroupId: 0,
	...
	function handleTodoCheck(item) {
		...
		// 等待动画完成后执行接口调用
		setTimeout(() => {
			http.put('/points-service/points/task/basics/updateStatus', {
				id: item.id,
				taskStatus: 1
			}).then(() => {
				// 从待完成列表中移除
