...
const getDishDetail = async (id) => {
  try {
    const res = await http.get(`/eat-service/eat/dishes/detail?id=${id}`)
...
const submitForm = async () => {
  ...
  try {
    if (formData.id) {
      ...
      await http.put('/eat-service/eat/dishes/update', requestData)
    } else {
      // 新增模式
      await http.post('/eat-service/eat/dishes/add', requestData)
    }
